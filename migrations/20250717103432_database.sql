...
/* Drop all triggers first (due to dependencies) */
DO
$$
    DECLARE
        t_name text;
    BEGIN
        FOR t_name IN (VALUES ('code'),
                              ...
                              ('verified_contracts'))
            LOOP
                EXECUTE format('DROP TRIGGER IF EXISTS update_set_updated_by ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS update_reuse_created_by ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS insert_set_updated_by ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS insert_set_created_by ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS update_set_updated_at ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS update_reuse_created_at ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS insert_set_updated_at ON %I', t_name);
                EXECUTE format('DROP TRIGGER IF EXISTS insert_set_created_at ON %I', t_name);
            END LOOP;
    END;
$$ LANGUAGE plpgsql;

/* Drop constraint from verified_contracts table */
ALTER TABLE verified_contracts DROP CONSTRAINT IF EXISTS runtime_transformations_json_schema;
ALTER TABLE verified_contracts DROP CONSTRAINT IF EXISTS creation_transformations_json_schema;
ALTER TABLE verified_contracts DROP CONSTRAINT IF EXISTS runtime_values_json_schema;
ALTER TABLE verified_contracts DROP CONSTRAINT IF EXISTS creation_values_json_schema;

/* Drop constraints from compiled_contracts table */
ALTER TABLE compiled_contracts DROP CONSTRAINT IF EXISTS runtime_code_artifacts_json_schema;
ALTER TABLE compiled_contracts DROP CONSTRAINT IF EXISTS creation_code_artifacts_json_schema;
ALTER TABLE compiled_contracts DROP CONSTRAINT IF EXISTS compilation_artifacts_json_schema;

/* Drop tables in reverse dependency order */
DROP TABLE IF EXISTS verified_contracts;
DROP TABLE IF EXISTS compiled_contracts_sources;
...
DROP FUNCTION IF EXISTS validate_runtime_transformations(jsonb);
DROP FUNCTION IF EXISTS validate_creation_transformations(jsonb);
DROP FUNCTION IF EXISTS validate_transformations(jsonb, text[]);
DROP FUNCTION IF EXISTS validate_transformations_call_protection(jsonb);
DROP FUNCTION IF EXISTS validate_transformations_cbor_auxdata(jsonb);
DROP FUNCTION IF EXISTS validate_transformations_immutable(jsonb);
DROP FUNCTION IF EXISTS validate_transformations_library(jsonb);
DROP FUNCTION IF EXISTS validate_transformations_constructor_arguments(jsonb);
DROP FUNCTION IF EXISTS validate_transformation_key_id(jsonb);
DROP FUNCTION IF EXISTS validate_transformation_key_offset(jsonb);
DROP FUNCTION IF EXISTS validate_transformation_key_type(jsonb, text);
DROP FUNCTION IF EXISTS validate_runtime_values(jsonb);
DROP FUNCTION IF EXISTS validate_creation_values(jsonb);
DROP FUNCTION IF EXISTS validate_values_call_protection(jsonb);
DROP FUNCTION IF EXISTS validate_values_cbor_auxdata(jsonb);
DROP FUNCTION IF EXISTS validate_values_immutables(jsonb);
DROP FUNCTION IF EXISTS validate_values_libraries(jsonb);
DROP FUNCTION IF EXISTS validate_values_constructor_arguments(jsonb);
DROP FUNCTION IF EXISTS validate_runtime_code_artifacts(jsonb);
DROP FUNCTION IF EXISTS validate_creation_code_artifacts(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_immutable_references(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_immutable_references_internal(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_link_references(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_link_references_internal(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_link_references_internal_file_libraries(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_cbor_auxdata(jsonb);
DROP FUNCTION IF EXISTS validate_code_artifacts_cbor_auxdata_internal(jsonb);
DROP FUNCTION IF EXISTS validate_compilation_artifacts(jsonb);
DROP FUNCTION IF EXISTS validate_compilation_artifacts_sources(jsonb);
DROP FUNCTION IF EXISTS validate_compilation_artifacts_sources_internal(jsonb);
DROP FUNCTION IF EXISTS validate_compilation_artifacts_abi(jsonb);
DROP FUNCTION IF EXISTS validate_json_object_keys(jsonb, text[], text[]);
DROP FUNCTION IF EXISTS is_valid_hex(text, text);
DROP FUNCTION IF EXISTS is_jsonb_null(jsonb);
DROP FUNCTION IF EXISTS is_jsonb_number(jsonb);
DROP FUNCTION IF EXISTS is_jsonb_array(jsonb);
DROP FUNCTION IF EXISTS is_jsonb_string(jsonb);
DROP FUNCTION IF EXISTS is_jsonb_object(jsonb);

/* Drop trigger functions */
DROP FUNCTION IF EXISTS trigger_set_updated_by();
DROP FUNCTION IF EXISTS trigger_reuse_created_by();
DROP FUNCTION IF EXISTS trigger_set_created_by();
DROP FUNCTION IF EXISTS trigger_set_updated_at();
DROP FUNCTION IF EXISTS trigger_reuse_created_at();
